...
- **可空类型**:
  - `Optional`是typing模块中的一个类型提示工具，它实际上是对类型的简化表示。使用 Optional[X] 等价于 Union[X, None]，意思是该值可以是类型 X 或者是 None。 
     - Optional[X] 表示一个可以是类型 X 或 None 的值。
     - 使用 Optional 可以提高代码的可读性和可维护性，特别是在函数参数和返回值中。
     - 在使用类型提示时，推荐使用 Optional 来明确变量的可空性。

- **None常量**:
  - `None`是一个特殊的常量，用于表示“无值”或“空值”。它是 NoneType 类型的唯一实例，通常用于指示变量没有值、函数没有返回值或者某些数据结构中缺少元素。
   - None 是表示“无值”或“空值”的特殊常量，其类型为 NoneType。
   - 常用于表示缺失值、默认参数和函数返回值。
   - 在条件判断中，None 被视为 False，并且应该使用 is 来进行比较。
  ```python
  value = None
  if value is None:
